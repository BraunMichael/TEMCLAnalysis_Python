...
def saveNNGraph(firstNNDict, pdfXValues, wavelength, timeString, suffix):
    if len(firstNNDict[wavelength]) > 3:
        ...
        fig.canvas.set_window_title('First NN Distance - ' + timeString + '_' + str(int(wavelength)) + 'nm')
        plt.title('First NN Distances - ' + timeString + '_' + str(int(wavelength)) + 'nm')
        setAxisTicks(ax)
        ax.margins(x=0)
        plt.xlabel('Distance (nm)')
        plt.ylabel('Probability')
        # plt.show()
        fig.savefig(os.path.join('OutputFigures', suffix+"_"+timeString+"_"+str(int(wavelength))+"nm"+'.png'), bbox_inches='tight')
        plt.close(fig)
