...
from lmfit.models import GaussianModel, PseudoVoigtModel, VoigtModel, LorentzianModel, Pearson7Model, ConstantModel
from bisect import bisect_left

multiRegionMin = 0.67
# multiRegionMax = 0.815
multiRegionMax = 0.825
indirectMin = 0.68
indirectMax = 0.73
directMin = 0.78
# directMax = 0.81
...
class SpectrumData:
    def __init__(self, xVals: list = None, intensity: list = None, nakedFileName: str = ''):
        ...
        self.bgSubIntensity = None  # Raw intensity
        self.lnBgSubIntensity = None  # ln of intensity
        self.expBgSubIntensity = None
...
def calculateSingleBackground(measuredAngles, intensities, angleNum, radiussquared, rollingBallRatioVal):
    centerAngle = measuredAngles[angleNum]
    centerIntensity = intensities[angleNum]
    ...
    backgroundDifference = ballPoints - intensities
...
def backgroundSubtractionPlotting(spectrumData: SpectrumData, rollingBall: RollingBall, setupOptions: SetupOptions):
    ...
    def update(_):
        ...
        if setupOptions.isLogPlot:
            subtracted.set_ydata(spectrumData.lnIntensity - np.log(rollingBallBackgroundDataUpdate))
        else:
...
def fittingRegionSelectionPlotting(spectrumData: SpectrumData, setupOptions: SetupOptions):
    ...
    indmin, indmax = np.searchsorted(spectrumData.xVals, (indirectMin, indirectMax))
    indmax = min(len(spectrumData.xVals) - 1, indmax)
    coordsDict = {'x': spectrumData.xVals[indmin:indmax], 'y': spectrumData.bgSubIntensity[indmin:indmax]}
    coordsList.append(coordsDict.copy())

    indmin, indmax = np.searchsorted(spectrumData.xVals, (directMin, directMax))
    indmax = min(len(spectrumData.xVals) - 1, indmax)
    coordsDict = {'x': spectrumData.xVals[indmin:indmax], 'y': spectrumData.bgSubIntensity[indmin:indmax]}
    coordsList.append(coordsDict.copy())

    indmin, indmax = np.searchsorted(spectrumData.xVals, (hiddenMin, hiddenMax))
    indmax = min(len(spectrumData.xVals) - 1, indmax)
    coordsDict = {'x': spectrumData.xVals[indmin:indmax], 'y': spectrumData.bgSubIntensity[indmin:indmax]}
    coordsList.append(coordsDict.copy())

    indmin, indmax = np.searchsorted(spectrumData.xVals, (multiRegionMin, multiRegionMax))
    indmax = min(len(spectrumData.xVals) - 1, indmax)
    coordsDict = {'x': spectrumData.xVals[indmin:indmax], 'y': spectrumData.bgSubIntensity[indmin:indmax]}
    multiRegionCoordsList.append(coordsDict.copy())
    ...
    for region in roiCoordsList:
        ...
        for entry in region:
            ...
            if modelType.lower() == 'gaussian':
                minimumFWHM = 0.01
                pars[prefixName + 'sigma'].set(min=minimumFWHM / 2*np.sqrt(np.log(2)*2))  # Converts from FWHM to sigma
            if modelType.lower() == 'pearsonvii':
                pars[prefixName + 'expon'].set(min=0.5 + np.finfo(float).eps)  # prevents negative values
            if modelType.lower() == 'voigt':
...
def plCalculationProcessing(spectrumData, centerXValsList, axs, setupOptions, out):
    ...
    spectrumData.altSNR = 1/np.mean(np.abs(out.residual)/out.best_fit)
...
def snContentFittingPlotting(spectrumData: SpectrumData, roiCoordsList: list, multiRegionCoordsList: list,
    ...
    if spectrumData.signalToNoise:
        ...
        print("Alt SNR: " + str(spectrumData.altSNR))
    if setupOptions.doBackgroundSubtraction:
...
def main():
    ...
    spectrumData.bgSubIntensity = spectrumData.intensity - spectrumData.background  # Store the background subtracted intensity (natural log) in the SpectrumData object
    spectrumData.lnBgSubIntensity = np.log(spectrumData.bgSubIntensity)  # Store the background subtracted intensity (as measured) in the SpectrumData object
